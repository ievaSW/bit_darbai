...
let full = `<i class="bi bi-star-fill"></i>`;
let half = `<i class="bi bi-star-half"></i>`;
let empty = `<i class="bi bi-star"></i>`;

let starsHTML = "";
const countoffullstars = Math.floor(rating);
halfstar = rating - countoffullstars >= 0.5,
emptystar = halfstar 
// ternerio operatorius reiskiantis if
? 4 - countoffullstars
// ternerio operatorius else
: 5 - countoffullstars;

for(let i= 0; i < countoffullstars; i++) starsHTML += full;
if(halfstar) starsHTML += half;
for(let i= 0; i < emptystar; i++) starsHTML += empty;
...
const Allproducts = ()=>{
  ...
  for (const product of products){
    ...
    <div id = "discount" class="discount">-${Math.round(product.discountPercentage) + "%"}</div>
        <img id = "Product_photo"  src="${product.thumbnail}" alt="">
      </div>
      <div class = "container">
          <h5 id="product_title">${product.title}</h5>
          <span class="stars" id="product_rating">${stars(product.rating)}
      ...
      <div>
            <div id="product_price">
                <span class="price"></span>${(product.price*((100-product.discountPercentage)/100)).toFixed(2)}<sup>${"$" + product.price.toFixed(2)}</sup>
                <button id="add_to_cart">Add to cart</button>
            </div>

        </div>
  </div>`
// product price yra kaina be nuolaidos
 }
